...
export class Group extends HasAttrs {
    ...
    *items() {
    ...
    create_dataset(name, data, shape, dtype, maxshape, chunks) {
        const final_dtype = dtype ?? guess_dtype(data);
        let metadata = dtype_to_metadata(final_dtype);
        if (!metadata.littleEndian) {
        ...
        if (metadata.vlen) {
            Module.create_vlen_str_dataset(this.file_id, this.path + "/" + name, prepared_data, final_shape, final_maxshape, final_chunks, metadata.type, metadata.size, metadata.signed, metadata.vlen);
        }
        else {
            let data_ptr = Module._malloc(prepared_data.byteLength);
            try {
                Module.HEAPU8.set(prepared_data, data_ptr);
                Module.create_dataset(this.file_id, this.path + "/" + name, BigInt(data_ptr), final_shape, final_maxshape, final_chunks, metadata.type, metadata.size, metadata.signed, metadata.vlen);
            }
            finally {
                Module._free(data_ptr);
            }
